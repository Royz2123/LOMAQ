...
# Base path for all the files
BASE_PATH = "./src/envs/traffic/small_grid/data/"
...
def main():
    try:
        ...
        print(f"[WARNING]: Directory already exists - {e}")

    # nod.xml file
    node = '  <node id="%s" x="%.2f" y="%.2f" type="%s"/>\n'
    write_file(BASE_PATH + 'exp.nod.xml', output_nodes(node))

    # typ.xml file
    write_file(BASE_PATH + 'exp.typ.xml', output_road_types())

    # edg.xml file
    edge = '  <edge id="%s" from="%s" to="%s" type="a"/>\n'
    write_file(BASE_PATH + 'exp.edg.xml', output_edges(edge))

    # con.xml file
    con = '  <connection from="%s" to="%s" fromLane="0" toLane="0"/>\n'
    write_file(BASE_PATH + 'exp.con.xml', output_connections(con))

    # tls.xml file
    tls = '  <tlLogic id="%s" programID="0" offset="0" type="static">\n'
    phase = '    <phase duration="%d" state="%s"/>\n'
    write_file(BASE_PATH + 'exp.tll.xml', output_tls(tls, phase))

    # net config file
    write_file(BASE_PATH + 'exp.netccfg', output_netconfig())

    # generate net.xml file
    os.system(f'netconvert -c {BASE_PATH}exp.netccfg')

    # raw.rou.xml file
    flow = '  <flow id="f_%s" from="%s" begin="%d" end="%d" vehsPerHour="%d" type="type1"/>\n'
    write_file(BASE_PATH + 'exp.raw.rou.xml', output_flows(flow, num_car_hourly=1000))

    # turns.xml file
    write_file(BASE_PATH + 'exp.turns.xml', output_turns())

    # generate rou.xml file
    os.system(f'jtrrouter -t {BASE_PATH}exp.turns.xml -n {BASE_PATH}exp.net.xml -r {BASE_PATH}exp.raw.rou.xml -o {BASE_PATH}exp.rou.xml')

    # add.xml file
    ild = '  <laneAreaDetector file="ild.out" freq="1" id="%s_%d" lane="%s_%d" pos="-50" endPos="-1"/>\n'
    write_file(BASE_PATH + 'exp.add.xml', output_ild(ild))

    # config file
    write_file(BASE_PATH + 'exp.sumocfg', output_config())
