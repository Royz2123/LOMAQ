...
def decompose(decomposer, batch):
    ...
    if decomposer.args.assume_binary_reward:
        global_rewards = global_rewards.long()
        local_rewards = RewardDecomposer.probs_to_reward(local_rewards)
...
def visualize_decomposer_1d(decomposer, batch, env_name="multi_particle"):
    ...
    if decomposer.args.assume_binary_reward:
        ys = [RewardDecomposer.probs_to_reward(reward_prob).detach().numpy() for reward_prob in ys]
...
def visualize_batch(batch, env_name="multi_particle"):
